...
const PaymentStatus = props => {
  ...
  const successMessage = <span>{THANK_YOU_PAGE.ORDER_PLACED_BEING_PROCESSED} {THANK_YOU_PAGE.EMAIL_SHIPPMENT_DETAILS} {THANK_YOU_PAGE.TRACK_YOUR} {THANK_YOU_PAGE.ORDER} <a className={styles['fontW600']} href={`/${country}/${language}/customer/orders/${props.orderId}`}>here</a>.</span>;
          ...
          <div className={`${styles['flx-spacebw-alignc']}`}>
            <h3 className={`${styles['mt-0']} ${styles['light-gry-clr']} ${styles['fontW600']}`}>{message}</h3>
            {
              props.status == "SUCCESSFUL" ?
                <a href={`/${country}/${language}/customer/orders`} className={styles['mb-20']}>
