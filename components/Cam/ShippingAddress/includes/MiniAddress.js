  ...
  return (
    <div className={`${styles.absolute} ${styles['bg-white']} ${styles['min-address-part']}`}>
      <div className={styles['min-address-list']}>
        {
          data && data.length > 1 && data.map((val, id) => {
            return (
              <div key={id} id={val.address_id} className={`${styles.flex} ${styles['flex-colum']} ${styles['p-10-20']} ${styles['min-address-part-inn']} ${styles.pointer}`} onClick={selectDeliverToAddress}>
                <h5 className={`${styles.fontW600} ${styles['m-0']} ${styles['mb-5']} ${styles['thick-gry-clr']} ${styles['text-capitalize']}`}> {`${val.first_name} ${val.last_name}`} </h5>
      ...
      <div className={`${styles.flex} ${styles['justify-center']} ${styles['p-10']} ${styles['add-new-addres']}`}>
        <button data-page="pdp" className={`${styles['fp-btn']} ${styles['fp-btn-default']} ${styles['left-radius']} ${styles['fp-btn-large']}`} onClick={showAddAdrress}> {DELIVERY_ADDR_PAGE.ADD_NEW_BTN} </button>
      </div>
    </div>
  );
};
