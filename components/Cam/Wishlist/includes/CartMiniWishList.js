      ...
      <div className={`${styles['view-wishlist-main']} ${styles['box']} ${styles['p-20']} ${styles['flex-center']}`}>
        <Col md={3}>
          <span className={styles['fs-12']}>
            {data.length === 1 ? `${CART_PAGE.THERE_IS} ${data.length} ${CART_PAGE.ITEM_IN_YOUR_WISHLIST}` : data.length > 1 ? `${CART_PAGE.THERE_ARE} ${data.length} ${CART_PAGE.ITEMS_IN_YOUR_WISHLIST}` : `${CART_PAGE.THERE_ARE} ${CART_PAGE.NO_PRODUCTS_IN_YOUR_FAVOURITES}`}
          </span>
        </Col>
        <Col md={7}>
          {
        ...
        <Col md={2} className={`${styles['pl-0']} ${styles['pr-0']} ${styles['flex']} ${styles['view-btn-list']}`}>
          <a className={`${styles['fp-btn']} ${styles['fs-12']} ${styles['fp-btn-default']} ${styles['text-uppercase']} ${styles['default-small']} ${styles['left-radius']}`} onClick={this.wishlistClickHandler}>{CART_PAGE.VIEW_WISHLIST}</a>
        </Col>
        }
      </div>
    )
