		...
		if (status === "INCREASED") {
			str = (
				<span className={`${styles["thick-red-clr"]} ${styles.flex}`}>
					<SVGComponent
						clsName={`${styles["alert-icon"]}`}
						src="icons/increase/increase"
					/>
					{`${WISH_LIST_PAGE.PRICE_INCREASED_BY} ${cur} ${changed_price.toFixed(
						2
					)}`}
				</span>
			);
			...
			str = (
				<span className={`${styles["success-green"]} ${styles.flex}`}>
					<SVGComponent
						clsName={`${styles["alert-icon"]}`}
						src="icons/decrease/decrease"
					/>
					{`${WISH_LIST_PAGE.PRICE_DECRECED_BY} ${cur} ${changed_price.toFixed(
						2
					)}`}
				</span>
			);
	...
	const routeChange = (
		...
		Router.push(
			`/${language}/pdp/${name
				...
				.toLowerCase()}/${
				tuin_id ? `${tuin_id}/` : ""
			}${listing_id}?pid=${product_id}&vid=${variant_id}&cid=${catalog_id}`
		);
	};
	return data.length === 0 ? (
		<div className={styles["no-wishlist-icon"]}>
			<div className={`${styles.flex} ${styles["no-wishlist-icon-inn"]}`}>
				<SVGComponent
					clsName={`${styles["deleno-wish-list-icon"]}`}
					src="icons/wish-list/no-wishlist"
				/>
				<h4
					className={`${styles["fs-24"]} ${styles["t-c"]} ${styles["pt-40"]}`}
				>
					{WISH_LIST_PAGE.NO_WISHLIST_LABEL}
				</h4>
				<div
					className={`${styles["fs-14"]} ${styles["thick-gry-clr"]} ${
						styles["pt-10"]
					}`}
				>
					{WISH_LIST_PAGE.DONT_WAIT_NO_MORE}
				</div>
				<a
					href={`/${language}`}
					className={`${styles["fp-btn"]} ${styles["fp-btn-primary"]} ${
						styles["fp-btn-x-large"]
					} ${styles["right-radius"]} ${styles["text-uppercase"]} ${
						styles.fontW600
					} ${styles["mt-40"]}`}
				>
					{WISH_LIST_PAGE.START_SHOPPING}
				</a>
			</div>
		</div>
	) : (
		<div className={`${styles["pl-5"]}`}>
			<div className={`${styles.flex}`}>
				<Col md={12} sm={12} xs={12} className={`${styles["pl-0"]}`}>
					<h4
						className={`${styles["mt-0"]} ${styles["mb-20"]} ${
							styles.fontW300
						}}`}
					>
						<span>{`Showing ${pageDetails.number * pageDetails.size + 1}-${
							(pageDetails.number + 1) * pageDetails.size >
							pageDetails.total_elements
								? pageDetails.total_elements
								: (pageDetails.number + 1) * pageDetails.size
						} of ${pageDetails.total_elements} ${
							WISH_LIST_PAGE.WISHLIST_HEADER
						}`}</span>
					</h4>
				</Col>
			</div>
				...
				{data.length > 0 &&
					data.map((item, index) => {
						...
						return (
							<div
								key={index}
								className={`${styles["thick-border-btm"]} ${
									styles["p-30-20"]
								} ${styles["mb-wishlist-part"]}`}
							>
								<Row className={styles["m-m-0"]}>
									<Col md={2} xs={2} className={styles["m-p-0"]}>
										<div
											...
											} ${styles.pointer} ${styles["mb-wishlist-part-img"]}`}
											onClick={() =>
												routeChange(
													variant_id,
													product_id,
									...
									<Col md={10} xs={10}>
										<Col md={8} xs={8} className={styles["pl-0"]}>
											<h5
												className={`${styles["mt-0"]} ${styles["mb-0"]} ${
													styles["thick-blue"]
												}`}
											>
												{brand_name}
											...
											<div
												className={`${styles["mt-30"]} ${styles["m-t-0"]} ${
													styles["m-fs-12"]
												}`}
											>
												{inventory_count > 0 ? (
													...
													<button
														...
														className={`${styles["fp-btn"]} ${
															styles["fp-btn-primary"]
														} ${styles["left-radius"]} ${styles["add-to-btn"]}`}
														onClick={notifyMe}
													>
										...
										<Col md={4} xs={4} className={styles["m-p-0"]}>
											<div
												className={`${styles.relative} ${styles.flex} ${
													styles["flex-colum"]
												} ${styles["price-details"]}`}
											>
												<span
													id={wishlist_id}
													className={`${styles.absolute} ${
														styles["delete-icon-part"]
													}`}
													onClick={deleteItem}
												>
													<SVGComponent
														clsName={`${styles["delete-icon"]}`}
														src="icons/delete-icon/delete-icon"
													/>
												</span>
												<h4
													className={`${styles.fontW600} ${
														styles["light-gry-clr"]
													} ${styles["mt-25"]} ${styles["ff-b"]}`}
												>
													<span
												...
												{variant_id && percentage(price, mrp) > 5 ? (
													...
													>
														<span
															className={`${styles["success-green"]} ${
																styles.flex
															}`}
														>
															{percentage(price, mrp)}%
														</span>
														&nbsp;&nbsp;&nbsp;
														<strike
															className={`${styles["label-gry-clr"]} ${
																styles["fs-12"]
															}`}
														>
															{cur} {mrp}
												...
												wishlisted_price &&
												wishlisted_price > 0 &&
												wishlisted_price.toString() !== price ? (
													<span className={`${styles["thick-gry-clr"]}`}>
														{WISH_LIST_PAGE.ITEM_WAS} {cur} {wishlisted_price}{" "}
														{WISH_LIST_PAGE.WHEN_ADDED_TO_WISHLIST}
													</span>
												) : (
													""
												)}
