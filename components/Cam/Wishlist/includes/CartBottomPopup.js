          ...
          <Slider {...settings} className={styles['cart-wishlist-popup-inn']}>
            {
              data.length > 0 && data.map((item, i) => {
                ...
                return (
                  ...
                  <Link route={`/${language}/pdp/${name.replace(/\//g, '').split(' ').join('-').toLowerCase()}/${listing_id}?pid=${product_id}&vid=${variant_id ? `${variant_id}` : ''}&cid=${catalog_id}`}>
                    ...
                    <Link route={`/${language}/pdp/${name.replace(/\//g, '').split(' ').join('-').toLowerCase()}/c/${listing_id}?pid=${product_id}&vid=${variant_id ? `${variant_id}` : ''}&cid=${catalog_id}`}>
