              ...
              data.length > 0 && data.map((item, i) => {
                ...
                return (
                  <div key={i} className={`${styles['item']} ${styles['flex']} ${styles['flex-colum']}`}>
                    <Link route={`/${language}/pdp/${name.replace(/\//g, '').split(' ').join('-').toLowerCase()}/${tuin_id ? `${tuin_id}/`: '' }${listing_id ? `${listing_id}`: ''}?pid=${product_id}&vid=${variant_id ? `${variant_id}` : ''}&cid=${catalog_id}`}>
