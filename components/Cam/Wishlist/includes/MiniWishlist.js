  ...
  return (
    <div className={`${styles['mini-wishlist']}`}>
              ...
              return (
                <div key={wishlist_id} className={`${styles.flex} ${styles['pt-15']} ${styles['pb-15']} ${styles['border-b']} ${styles['min-items-list']}`}>
                  <Col md={2} sm={2} xs={2} className={`${styles['pl-0']} ${styles['pr-0']}`}>
                    ...
                    <Link route={`/${language}/pdp/${name.replace(/\//g, '').split(' ').join('-').toLowerCase()}/${tuin_id ? `${tuin_id}/`: '' }${listing_id}?pid=${product_id}&vid=${variant_id}&cid=${catalog_id}`}>
                  ...
                  <Col md={10} sm={10} xs={10} className={`${styles['pr-0']} ${styles['wishlist-details-labels']} ${styles.flex} ${styles['justify-between']} ${styles['flex-colum']}`}>
                    <div className={`${styles['flx-space-bw']}`}>
                      ...
                      <Link route={`/${language}/pdp/${name.replace(/\//g, '').split(' ').join('-').toLowerCase()}/${tuin_id ? `${tuin_id}/`: '' }${listing_id}?pid=${product_id}&vid=${variant_id}&cid=${catalog_id}`}>
                          <a className={`${styles.width100} ${styles['ht-100P']} ${styles['fs-12']} ${styles['mr-5']} ${styles['light-gry-clr']}`}>
                            {name}
                          </a>
                        </Link>
                        {
                    ...
                    <div>
                      <div className={`${styles.fontW600} ${styles['pt-10']} ${styles['fs-14']} ${lang === 'ar' ? styles['arbic-direction-rev'] : ''}`}>
      ...
      {props.wishListCount && props.wishListCount > 0 ?
        <Col md={12} className={`${styles['wishlist-background']} ${styles['p-20']}`}>
          <Button
