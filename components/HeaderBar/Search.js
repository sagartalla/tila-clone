...
class Search extends Component {
  ...
  submitQuery(e) {
    ...
    window.scrollTo(0, 0);
    Router.pushRoute(`/${country}/${language}/search?q=${this.state.query.trim()}&qs=${this.state.query ? true : false}&POS=${searchPosition ? searchPosition : null}&${Object.entries(this.props.optionalParams).map(([key, val]) => `${key}=${val}`).join('&')}`);
  }
