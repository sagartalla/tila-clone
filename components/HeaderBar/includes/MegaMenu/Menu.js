      ...
      return (
        <li className={`${styles['megamenu-sub-list']} ${depth === 2 ? styles['pl-20'] : null}`} key={childItem.id} onClick={this.props.onLinkClick}>
          <span className={`${styles['flex']}`}>
            ...
            <Link route={`/${language}/search/${childItem.displayName ? childItem.displayName.split(' ').join('-').toLowerCase(): ''}?categoryTree=true&isListed=false&sid=${this.props.parentID},${childItem.id}`}>
