...
class OrderReturnExchange extends Component {
  ...
  render() {
    const {
      query, orderIssue, orderDetails, orderData,
    } = this.props;
                  ...
                  <Row>
                    <Col md={6}>
                      <div className={styles['product-wrap']}>
                        <Col md={2}>
                          <Link route={`/${language}/pdp/${name && name.replace(/\//g, '').split(' ').join('-').toLowerCase()}/${tuin ? `${tuin}/`: '' }${listing_id ? `${listing_id}`: ''}?pid=${product_id}&vid=${variant_id}&cid=${catalog_id}`}>
                            <a target="_blank">
