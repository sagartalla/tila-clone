...
class OrderReturnExchange extends Component {
  ...
  render() {
    ...
    const { product_id, catalog_id } = variant_info;
    return (
      <div>
        <Grid>
          <Row>
                  ...
                  <Row>
                    <Col md={6}>
                      <div className={styles['product-wrap']}>
                        <Col md={2}>
                          <Link route={`/${language}/pdp/${name && name.replace(/\//g, '').split(' ').join('-').toLowerCase()}/${listing_id}?pid=${product_id}&vid=${variant_id}&cid=${catalog_id}`}>
                            <a target="_blank">
