...
class AddToCart extends Component {
	...
	render() {
		...
		return (
			<div
				id="cart-btn-cont"
				className={`${styles["addto-cart"]} ${styles["addto-cart-lang"]} ${
					styles[styling]
				}`}
				style={{ top: `${top}px` }}
			>
				<React.Fragment>
					...
					{(availabilityError || stockError) &&
					(shippingInfo === null || shippingInfo.shippable) ? null : (
						<div
							className={`${styles["pt-10"]} ${styles["flx-space-bw"]} ${
								styles["ipad-p-0"]
							}`}
						>
							<ViewportTrackerHOC
								clickEvent={`PDP-ADD-TO-CART-CLICK`}
								disableViewportTracking={true}
							>
								<div data-listing-id={listingId} style={{ width: "45%" }}>
									<Button
										className={`${styles["fs-16"]} ${styles["ipad-fs-14"]} ${
											styles["add-to-card-btn"]
										} ${styles["text-uppercase"]} ${styles["flex"]}`}
										disabled={isLoading || isAddedToCart}
										onClick={
											isAddedToCart === false ? this.addToCart : undefined
										}
