...
class Display extends Component {
	...
	getUrl = crum =>
		`/${language}/search/${crum.display_name_en
			.toLowerCase()
			.replace(/\//g, "")
			.split(" ")
			.join("-")}?q=${crum.display_name_en}&isListed=true&sid=${crum.id}`;
			...
			if (!isLoggedIn) {
				...
				addToWishlistAndFetch({
					...
					wishlisted_price: offerPricing.showPrise
						? offerPricing.showPrise.display_value
						: null,
					wishlisted_currency: offerPricing.showPrise
						? offerPricing.showPrise.currency_code
						: offerPricing.currency
				});
			}
		}
	}
	...
	pauseVideo = () => {
		...
		const videos = Object.values(
			document.getElementsByClassName(`V_${product_id}`)
		);
		...
		return (
			<div className={`${styles["ht-100per"]} ${styles["mobile-slide-part"]}`}>
						...
						{breadcrums.map((crum, index) => (
							<span key={crum.id}>
								...
								{breadcrums.length - 1 !== index && (
									<span className={`${styles["label-gry-clr"]}`}>
										&nbsp;&nbsp;{">"}&nbsp;&nbsp;
									</span>
								)}
							</span>
				...
				<div className={`${styles["display-item-wrap"]} ${styles.relative}`}>
					...
					>
						<div data-listing-id={listingId} data-is-wishlisted={isWishlisted}>
							<a onClick={this.addToWishlist}>
								<SVGComponent
									clsName={`${styles["wishlist-icon"]}`}
									src={
										isWishlisted
				...
				<div
					className={`bottom-sliderDiv ${styles["bottom-slider"]} ${
						styles.flex
					} ${styles["border-t"]}`}
				>
					<Col md={4} sm={4}>
								...
								imgs.map(({ url, type }) => (
									<div className={styles["carousel-item-wrap"]} key={url}>
										{type === "IMAGE" ? (
											...
											<SVGComponent
												clsName={styles["carousel-item-wrap-inn"]}
												src="icons/play_video"
											/>
										)}
									</div>
								))}
						...
						<Fragment>
							{extraOffers[0] ? (
								<Col md={4} sm={4}>
									...
									>
										<h5 className={`${styles["mb-5"]} ${styles.fontW600}`}>
											{extraOffers[0]}
										</h5>
