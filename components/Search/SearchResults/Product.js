...
class Product extends Component {
  ...
  routeChange(productId, variantId, catalogId, itemtype, index, pageNum) {
    ...
    // Router.pushRoute(`/${language}/pdp/${displayName && displayName.split(' ').join('-').toLowerCase()}/c/${catalogId}/p/${productId}/l/${listing_id}/v/${variants.length > 0 && variants[selectedIndex].variantId ? `${variants[selectedIndex].variantId}` : ''}`)
  }
