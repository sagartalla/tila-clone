  ...
  render() {
    ...
    return (
      <Fragment>
        ...
        >
          <Link route={`/${language}/pdp/${displayName.replace(/\//g, '').split(' ').join('-').toLowerCase()}/${tuinId ? `${tuinId}/`: '' }${listing_id ? `${listing_id}`: 'oos'}?pid=${product_id}&vid=${variant_id}&cid=${catalog_id}`}>
            <a target="_blank">
