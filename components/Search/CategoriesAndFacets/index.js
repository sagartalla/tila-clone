    ...
    return (<ul>
      ...
      }
      {
        filters.facets.map((filter) => {
          return <CheckboxFacet filter={filter} key={filter.id} onChangeHandle={this.onChangeHandle(filter.name)} selectedFilters={facets[filter.name] || []}/>
        })
      }
    </ul>);
  }
