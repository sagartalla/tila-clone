                ...
                return (
                  <div className={styles.item} key={data.productId}>
                    ...
                    <div className={styles['item-label']}>
                      <a title={data.displayName} className={`${styles.ellips} ${styles.width100} ${styles['black-color']} ${styles['fs-12']}`} href={`/${language}/pdp/${displayName.replace(/\//g, '').split(' ').join('-').toLowerCase()}/${tuin ? `${tuin}/`: '' }${listing_id ? `${listing_id}`: ''}?pid=${product_id}&vid=${variant_id}&cid=${catalog_id}`}>
                        {data.displayName}
                      </a>
                    </div>
