...
const VaultCard = (props) => {
  ...
  return (
    <div className={`${styles['p-10-20']} ${styles['border-b']} ${styles.relative} ${vaultResults.length > 1 && styles['instant-check-list-inn']}`} onClick={toggleMiniVault}>
      <h5 className={`${styles['mt-5']} ${styles['mb-5']} ${styles['card-number']}`}>{defaultCard && defaultCard.masked_number && defaultCard.masked_number.replace(/(.{4})/g, '$1 ')}</h5>
      <div className={`${styles['flx-space-bw']} ${styles['fs-12']} ${styles['thick-gry-clr']}`}>
        <span>{defaultCard && defaultCard.holder_name}</span> <span>{defaultCard && defaultCard.expiry_month}/{defaultCard && defaultCard.expiry_year}</span>
      </div>
