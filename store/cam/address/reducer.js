...
const shippingAddrReducer = typeToReducer({
  ...
  [actions.SEND_NEW_ADDR_DETAILS]: {
    ...
    FULFILLED: (state, action) => {
      ...
      const tempState =  state.data ? state.data : state; // WHY ternary operator. check it [TODO]
      
      return Object.assign(
        {}, 
        state, 
        { 
          data: _.values( Object.assign( {}, tempState, { [Object.keys(tempState).length+1] : addr_id_add } ) ),
           ui: { loading: true } 
        });
    },
  },
