...
const getWishListResults = (store) => {
  if (store.wishlistReducer.data) {
    ...
    data && data.length > 0 && data.forEach((item) => {
      ...
      const tuinId = item && item.product_details && item.product_details.product_details_vo && item.product_details.product_details_vo.cached_variant ? item.product_details.product_details_vo.cached_variant[variant_key].attribute_map && item.product_details.product_details_vo.cached_variant[variant_key].attribute_map.tuin.attribute_values[0].value : null;
