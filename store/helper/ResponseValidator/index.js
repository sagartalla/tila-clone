...
function recurseAndCheckType(expectedFormat, exisitingFormat, canBeEmpty, isOptional, traceKey='') {
  ...
  if(expectedType !== exisitingType) {
    stack.push(`Expected: ${expectedType} Got: ${exisitingType} for key: ${traceKey}`);
    return false;
  }
  if(expectedType === 'Object') {
    ...
    for(var i = 0; i < keys.length; i++) {
      ...
      if(keys[i] === 'RANDOM_ID') {
        const innerKeys = Object.keys(exisitingFormat);
        const values = Object.values(exisitingFormat);
        for(var j = 0; j < values.length; j++) {
          if(!isValid) {
            break;
          }
          isValid = isValid && recurseAndCheckType(expectedFormat[keys[i]].VALUE_TYPE, values[j], expectedFormat[keys[i]].canBeEmpty, expectedFormat[keys[i]].isOptional, `${traceKey}.${innerKeys[j]}`);
        }
      } else {
        ...
        isValid = isValid && recurseAndCheckType(passingValue, exisitingFormat[keys[i]], expectedFormat[keys[i]].canBeEmpty, expectedFormat[keys[i]].isOptional, `${traceKey}.${keys[i]}`);
      }
    }
    return isValid;
    ...
    for(var i = 0; i < exisitingFormat.length; i++) {
      ...
      isValid = isValid && recurseAndCheckType(expectedFormat[i], exisitingFormat[i], expectedFormat[i].canBeEmpty, expectedFormat[i].isOptional, `${traceKey}.${i}`);
    }
    return isValid;
  } else {
     return true;
